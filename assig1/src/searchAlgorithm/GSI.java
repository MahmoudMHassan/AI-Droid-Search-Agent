...
public class GSI implements GenericSearchAlgorithm {
	...
	public GSI(HelpR2D2 problem, boolean visualize) {
		queue = new ArrayList<Node>();
		this.problem = problem;
		this.node = new Node(problem.getInitialState(), null, "GSI");
		queue.add(node);
	...
	public ArrayList<Node> search() {
		...
		//	System.out.println(node);
			...
			for (int i = 0; i < states.size(); i++) {
				Node node1 = new Node(states.get(i), node, "GSI");
				queue.add(node1);
				nodes.add(node1);
			}
